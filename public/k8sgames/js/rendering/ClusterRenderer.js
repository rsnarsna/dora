...
export class ClusterRenderer {
    ...
    _initGrid() {
        ...
        // Visual 3D Boundary Wireframe Box (Landing Space edges)
        const maxHeight = 15;
        const boundaryGeometry = new THREE.BoxGeometry(GRID_SIZE, maxHeight, GRID_SIZE);
        ...
        const boundaryLines = new THREE.LineSegments(edges, boundaryMaterial);
        boundaryLines.position.y = maxHeight / 2;
        this.gridGroup.add(boundaryLines);
    ...
    _handleMouseMove(event) {
        ...
        if (this._draggingResource) {
            ...
            if (group) {
                const limit = GRID_SIZE / 2; // 20
                const maxHeight = 15;

                if (event.shiftKey) {
                    const deltaY = -event.movementY * 0.05;
                    // Clamp Y between 0 (ground) and maxHeight (ceiling)
                    group.position.y = Math.max(0, Math.min(maxHeight, (group.position.y || 0) + deltaY));
                    ...
                    if (pos) {
                        // Clamp X and Z within grid boundaries
